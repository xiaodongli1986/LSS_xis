...
\affil{School of Physics, Korea Institute for Advanced Study, 85 Heogiro, Dongdaemun-gu, Seoul 130-722, Korea}
\and
\author{Stephen Appleby, Sungwook E. Hong}
\affil{School of Physics, Korea Institute for Advanced Study, 85 Heogiro, Dongdaemun-gu, Seoul 130-722, Korea}
...
\begin{abstract}
We propose to use the redshift of the shape of galaxy angular 2-point correlation function (2pCF) to constrain cosmology.
The method is a continue of our series of works \citep{Li2014,Li2015,Li2016} probing the redshift dependence of Alcock-Paczynski effect and volume effect. 
A wrongly adopted cosmology result in redshift-dependent mis-scaling in the constructed galaxy distribution,
which leads to redshift-dependent rescaling in the measured angular 2pCF.
We test our method on the HR4 mock galaxies having 457, 406, 352, 206 and 228 million galaxies at redshfits 0, 0.5, 1, 1.5, 2,
and find the redshift evolution of the scaling of angular 2pCF is sensitive to the adopted cosmological parameters and 
relatively insensitive to the gravitational growth of structure,
...
The method could be applicable to future photometric surveys of DES, LSST to derive tight cosmological constraints.
...
Dark energy has become the most important scientific issue since the discovery of cosmic acceleration \citep{Riess1998,Perl1999}.
Many theoretical and observational effect has been made \citep{Weinberg,Li2011,DHW2013}, but its properties still remain far from accurately known and its nature still remains mystery. 
...
To measure the properties dark energy we need accurate probe of the cosmic expansion history. 
In large scale structure surveys the two representative geometrical observables are the angular diameter distance $D_A$ and the Hubble factor, $H$. 
If these can be precisely measured as functions of redshift, then tight constraints can be placed on the parameters of the cosmological model,
...
These distortion include volume effect which is the alteration of the comoving volume size of structures,
and the Alcock-Paczynski (AP) effect, induced by the fact that distances along and perpendicular to the line of sight are fundamentally different.
...
A series of statistically method has been proposed to measure the AP test in the LSS
\citep{Ballinger1996,Matsubara1996,Marinoni2010,Ryden1995,LavausWandelt1995}
and have been applied to a series of LSS surveys \citep{Outram2004,Blake2011,ChuangWang2012,
Reid2012,Beutler2013,Linder2013,2014arXiv1407.2257S, Jeong2014,Sutter2014,2014ApJ...781...96L,
Alam2016, Beutler2016, Sanchez2016,Qingqing2016}. 
The probe of volume effect is usually probed through number counting of 
galaxy clusters \citep{PS1974,VL1996}, topology \citep{topology};
it is also part of the physical effects measured in the measurements of the BAO scale \citep{some reference},
or the probe of the shape of 2pCF and power spectrum \citep{some reference}.


In \cite{Li2014,Li2015,Li2016} we proposed a series of methods to probe the geometric information encoded in the LSS data.
We make use of the fact that the AP or volume effect commonly introduce redshift-dependent geometric distorion in the LSS,
which can be distinguished from the other effects such as the redshfit space distortion (RSD).
RSD arises because the radial distances of galaxies are inferred from redshifts which is contaminated by the pecular motion of galaxies, 
leading to apparent anisotropy even if the adopted cosmology is correct \citep{Ballinger1996}.
RSD is the usually main caveats for probe geometry using LSS,
and we found it can be largerly avoid by focusing on the the redshift dependence of the anisotropy.

\cite{Li2014} proposed used the {\it galaxy density gradient field} 
to characterize the anisotropies in LSS and tested the idea on Horizon Run 3 (HR3) N-body simulations \citep{horizonrun},
showing the redsift evolution of the gardient field can be used to infer unbiased constraints cosmological parameters and is insensitive to RSD.
The same topic was revisited in \cite{Li2015}, but using the statistical tool of {\it galaxy two-point correlation function} (2pCF) as a function of angle, $\xi(\mu)$.
When incorrect cosmological parameters are adopted, the shape of $\xi(\mu)$ appears anisotropic due to the AP effect, 
and the amplitude is shifted by the change in comoving volume;
both effects have significant redshift dependence.
The RSD effect, although significantly distorting $\xi(\mu)$, 
exhibits much less redshift evolution compared to the effect coming from incorrectly adopted cosmologies.
The method was applied to BOSS DR12 galaxies \citep{Li2016} to probe the redshift dependent of AP effect and lead to tight cosmological constraints.

We developed a method measuring the redshift of the shape of galaxy angular correlation function to constrain cosmology.
A wrongly adopted cosmology result in redshift-dependent mis-scaling in the constructed galaxy distribution,
which leads to redshift-dependent change in the angular 2pCF shape.
...
We test our method on the HR4 mock galaxies having hundreds of million galaxies at redshfits 0-2.
Analyzing redshift evolution of the shape of 2pCF on scales $5 - 40 h^{-1} {\rm Mpc}$, we derive tight constraints on $\Omega_m$ and $w$.
...
In \S 2 we briefly review the nature and consequences of the geometric distortion in LSS when performing coordinate transforms in a cosmological context. 
In \S 3 we describe the mock galaxies produced from N-body simulations which are used to test our methodology.
...
Different from the large variation of amplitude among the different redshifts, %The shape of $r_\perp\omega$ also evolves with redshift.
...
The mis-scaling not only changes the of the 2pCF but also modifies its amplitude. %The amplitude of $r_\perp\omega$ is amplified due to the stretch of scale.
...
the only effect of RSD come in the procedure of sample preparation. %preparing the subsamples of galaxies for 2pCF analysis.
