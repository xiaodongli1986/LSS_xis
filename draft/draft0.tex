...
The Z direction with thickness $105\ h^{-1}\rm Mpc$ is treated as the radial direction and the X, Y direction with width $1575 h^{-1}\ \rm Mpc$ are treated as the angular plane.
In our simulation box we can have 120 such subsamples.
...
\begin{equation}
 \hat \omega_{\rm wrong}(r) = \hat\omega_{\rm true}(\alpha_{\perp} r),
\end{equation}
a simple consequence of the fact that clustering patterns at distance $r$ now appears on a the scale $\alpha_{\perp} r$.
...
The middle panel displays the clear redshift evolution of shape of $r_\perp\omega$.
E.g, from $z=0$ to $z=2$,
...
at higher redshift, 
there is a 20-40\% change in the value of $\hat {r_\perp\omega}$.

The amplitude of $r_\perp\omega$ is amplified due to the stretch of scale.
...
In all cases, the scaling leads to evident redshift evolution of the shape of $r_\perp\omega$.
...
This leads to ``steeper'' slope of $r_\perp\omega$, 
...
while the change in $\hat{r_\perp\omega}$ is only $0.5\%,\ 2\%$ and $4\%$.
...
The redshift evolution is characterized by the difference between the $\hat{r_\perp\omega}$ measured in the lowest redshift and higher ones;
the cosmology having least redshift evolution of $\hat{r_\perp\omega}$ is considered to be the most plausible one and has smallest $\chi^2$.
So we have
...
is number of bins in $\hat{r_\perp\omega(r)}$;
...
$\hat {r_\perp\omega}$, with systematic effects subtracted
...
E.g., if for the $z=0$ snapshot we use the $\hat{r_\perp\omega}$ measured in the subsample $0h^{-1}{\rm Mpc}<Z<105 h^{-1}{\rm Mpc}$,
at higher redshifts we adopt the $\hat{r_\perp\omega}$ measured in a subsample $105h^{-1}{\rm Mpc}<Z<210 h^{-1}{\rm Mpc}$,
...
one would significantly underestimate the statistical undertainty of $\delta \hat{r_\perp\omega}$ by ignoring the cosmic variance
...
the stretch of structure leads to shallower slope of $\hat{r_\perp\omega}$ and, the higher the redshift, the shallower.
In all cosmologies we find significant detection of redshift evolution of $\hat{r_\perp\omega}$ (in the lowest panels, $\delta \hat{r_\perp\omega}\neq0$).
