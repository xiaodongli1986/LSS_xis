...
\altaffiltext{1}{Corresponding Author: Cris, Add your email!!!}
...
\begin{abstract}
We propose to use the redshift dependence of the galaxy angular 2-point correlation function (2pCF)
to constrain cosmological parameters.
When an incorrect set of parameters is assumed for the coordinate transformation from redshift space to comoving space, 
it produces {\it redshift-dependent mis-scaling} in the constructed galaxy distribution.
As a result, we observe a redshift-dependent rescaling of the measured galaxy angular 2pCF.
We test our method on the Horizon Run 4 N-body simulation snapshot data, 
which contains hundreds of millions of galaxies at redshifts 
0, 0.5, 1, 1.5 and 2
We find the shape of angular 2pCF, as a function of scale,
exhibits evident redshift evolution when an incorrect set of cosmological parameters adopted,
and is relatively insensitive to the other effects such as gravitational growth of structure,
...
The method could be applicable to future large scale structure surveys, especially the photometric surveys such as DES, LSST, 
to derive tight cosmological constraints.
This work is a continuation of our previous works \cite{Li2014,Li2015,Li2016},
which use a strategy to constraining cosmological parameters based on some redshift-invariant physical quantity;
they have a lot of merit,
e.g., lead to powerful constraints, are less affected by the systematic effects introduced by non-linear clustering or RSD,
do not require accurate analytical modeling of the galaxy correlation function,
are applicable on relatively small clustering scales,
and are applicable to many future LSS surveys.
\end{abstract}
...
the higher the redshift, the greater the enhancement.%the amplitude monotonically increases as a function of redshift.
...
are imposed to create samples with different galaxy bias.
...
but the shape of the \scr{normalised} 2pCF remains less affected (middle panel).
...
with different redshift ranges. So that we can get the 2pCF measurement at different redshfits.
...
The lower panel of Figure \ref{fig_sys} shows how it affects on the 2pCF.
...
But the redshift evolution of $\hat\omega_{r_\perp}$ is still small, remain $\lesssim4\%$ at $0.5\leq z \leq 2$.
So RSD should not very affect the application of our method as long as its redshift evolution is small;
the small effect of RSD can be precisely modeled by simulations and corrected.
...
This effect should be properly quantified when dealing with real observational data.
...
the compression shifts the clustering patterns into smaller scales
...
one obtains very small 1$\sigma$ uncertainties of $\delta\Omega_m\approx0.002,\delta w\approx0.01$.


\section{Concluding Remarks}
 
We developed a simple method of measuring the redshift of the shape of the galaxy angular correlation function, $\hat \omega_{r_\perp}$, to constrain cosmology.
