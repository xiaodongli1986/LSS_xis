...
%DIF DEL draft3.1_cris.tex   Tue Apr  4 12:48:12 2017
%DIF ADD draft3.3.tex        Sun Apr  9 15:57:06 2017
%%
...
\begin{abstract}
We \DIFdelbegin \DIFdel{propose }\DIFdelend \DIFaddbegin \DIFadd{develop a methodology }\DIFaddend to use the redshift dependence of the galaxy 2-point correlation function across the line-of-sight (2pCF), $\xi(r_{\bot})$, \DIFdelbegin \DIFdel{to constrain }\DIFdelend \DIFaddbegin \DIFadd{as a probe of }\DIFaddend cosmological parameters. 
\DIFdelbegin \DIFdel{When an incorrect set of cosmological parameters is assumed for transforming galaxy redshifts to comoving distances, 
it produces }\DIFdelend \DIFaddbegin \DIFadd{The positions of galaxies in comoving Cartesian space varies under different cosmological parameter choices, 
inducing a }\DIFaddend {\it redshift-dependent \DIFdelbegin \DIFdel{mis-scaling}\DIFdelend \DIFaddbegin \DIFadd{scaling}\DIFaddend } in the galaxy distribution. 
\DIFdelbegin \DIFdel{As a result, we observe }\DIFdelend \DIFaddbegin \DIFadd{This geometrical distortion can be observed as }\DIFaddend a redshift-dependent rescaling in the measured $\xi(r_{\bot})$. 
\DIFdelbegin \DIFdel{Using mock galaxy catalogues (containing hundreds of millions of }\DIFdelend \DIFaddbegin \DIFadd{We test this methodology using a sample of 1.75 billion mock }\DIFaddend galaxies at redshifts 0, 0.5, 1, 1.5, 2\DIFdelbegin \DIFdel{) }\DIFdelend \DIFaddbegin \DIFadd{, 
drawn }\DIFaddend from the Horizon Run 4 N-body simulation\DIFdelbegin \DIFdel{, we find that the redshift evolution of the }\DIFdelend \DIFaddbegin \DIFadd{.
The shape of }\DIFaddend $\xi(r_{\bot})$ \DIFdelbegin \DIFdel{shape, as a function of scale,
is 
sensitive to the adopted cosmological parameters. 
%DIF <  mock galaxy catalogues having hundreds of millions of galaxies at redshifts 0, 0.5, 1, 1.5, 2.
However, it is relatively insensitive to }\DIFdelend \DIFaddbegin \DIFadd{can exhibit a significant redshift evolution when the galaxy sample is 
analyzed under a cosmology differing from the true, simulated one. 
Other contributions, including }\DIFaddend the gravitational growth of structure, 
\DIFdelbegin \DIFdel{the }\DIFdelend galaxy bias, and the \DIFdelbegin \DIFdel{effect of }\DIFdelend redshift space distortions\DIFdelbegin \DIFdel{.
Analyzing the 2pCF shapeon scales $5  h^{-1} {\rm Mpc} \leq r_\perp \leq 40 h^{-1} {\rm Mpc}$, 
we derive tight constraints on $\Omega_m$ and $w$ with 68.3\% CL uncertainties of 0.015 and 0.05. 
%DIF < Our method is complementary to current works using 2pCF to constrain cosmological parameters.
...
}\DIFaddend This method could be applicable to future large scale structure surveys, 
...
\DIFdelbegin \DIFdel{They are all powerful in constraining cosmology and have many merits,
...
}\DIFdelend %DIF > We propose to use the redshift dependence of the galaxy 2-point correlation function across the line-of-sight (2pCF), $\xi(r_{\bot})$, to constrain cosmological parameters.
%DIF > When an incorrect set of cosmological parameters is assumed for transforming galaxy redshifts to comoving distances, 
%DIF > it produces {\it redshift-dependent mis-scaling} in the galaxy distribution.
%DIF > As a result, we observe a redshift-dependent rescaling in the measured $\xi(r_{\bot})$.
%DIF > We test this idea using a sample of 1.75 billion mock galaxies at redshifts 0, 0.5, 1, 1.5, 2,
%DIF > drawn from the Horizon Run 4 N-body simulation.
%DIF > The shape of $\xi(r_{\bot})$ exhibits redshift evolution when the adopted cosmology is wrong,
%DIF >  mock galaxy catalogues having hundreds of millions of galaxies at redshifts 0, 0.5, 1, 1.5, 2.
%DIF > while the other effects, including the gravitational growth of structure,
%DIF > galaxy bias, and the redshift space distortions, do not produce large redshift evolution in the shape.
%DIF > Analyzing the 2pCF shape on scales $20  h^{-1} {\rm Mpc} \leq r_\perp \leq 40 h^{-1} {\rm Mpc}$, 
%DIF > we derive tight constraints on $\Omega_m$ and $w$ with 68.3\% CL uncertainties of 0.015 and 0.05.
%DIF > We show that one can make use of this fact to constrain the values of cosmological parameters governing the 
%DIF > expansion history of the universe.
%DIF > Our method is complementary to current works using 2pCF to constrain cosmological parameters.
%DIF > This method could be applicable to future large scale structure surveys, 
%DIF > especially photometric surveys such as DES, LSST, 
%DIF > to derive tight cosmological constraints.
%DIF > This work is a continuation of our previous works \citep{Li2014,Li2015,Li2016}
%DIF > as a strategy to constrain cosmological parameters using redshift-invariant physical quantities.
%DIF > They are all powerful in constraining cosmology and have many merits,
%DIF > e.g., they are less affected by the systematic effects induced by non-linear clustering or RSD,
%DIF > they do not require accurate modeling of galaxy clustering,
%DIF > and are applicable on relatively small clustering scales.
\end{abstract}
...
In an effort to probe the cosmic expansion history\DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend large scale structure (LSS) surveys \DIFdelbegin \DIFdel{enable measurements of two geometrical quantities, 
}\DIFdelend \DIFaddbegin \DIFadd{are utilized to  measure two key geometrical quantities; 
}\DIFaddend the angular diameter distance $D_A$ and the Hubble factor, $H$. 
If they were precisely measured as a function of redshift, 
\DIFaddbegin \DIFadd{then }\DIFaddend tight constraints can be placed on cosmological parameters, 
e.g. the matter density $\Omega_m$ and the equation of state (EoS) of dark energy $w$.

When an incorrect cosmological model is assumed \DIFdelbegin \DIFdel{when }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend transforming galaxy redshifts to comoving distances,
geometric distortions are \DIFdelbegin \DIFdel{produced }\DIFdelend \DIFaddbegin \DIFadd{induced }\DIFaddend in the resultant galaxy distribution. 
It should be noted that this effect is not related to \DIFdelbegin \DIFdel{galaxy peculiar velocities which could cause redshift space distortions }\DIFdelend \DIFaddbegin \DIFadd{the peculiar velocities of galaxies, which cause redshift-space distortions (RSD)}\DIFaddend .
...
%DIF < Measuring the ratio of galaxy clustering in the radial and transverse directions, provides a probe of the Alcock-Paczynski (AP) effect \citep{AP1979}.
%DIF < There have been a series of works studying the application of the AP test to the LSS
...
%DIFDELCMD < %%%
\DIFdel{However, there have also been alternative approaches in utilising }\DIFdelend \DIFaddbegin \DIFadd{Alternative approaches include utilizing }\DIFaddend the AP effect using the symmetry properties of galaxy pairs \citep{Marinoni2010,Jennings2011,BB2012}
and cosmic voids \citep{Ryden1995,LavausWandelt1995,Sutter2014,Qingqing2016}.
\DIFdelbegin \DIFdel{Methods proposed }\DIFdelend \DIFaddbegin 

\DIFadd{There have also been studies proposing }\DIFaddend to measure the volume effect \DIFaddbegin \DIFadd{of this geometrical rescaling, 
these }\DIFaddend include the number counting of galaxy clusters \citep{PS1974,VL1996}, 
...
\cite{Li2014,Li2015,Li2016} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Li2014,Li2015} }%DIFAUXCMD
}\DIFaddend we developed a novel strategy to probe $D_A$ and $H$ from the LSS data. 
%DIF < proposed a series of methods to probe the geometric information encoded in the LSS data.
We found that the AP \DIFdelbegin \DIFdel{or volume effect in general }\DIFdelend \DIFaddbegin \DIFadd{effect }\DIFaddend introduces a geometric distortion that evolves significantly with redshift;
this phenomenon can be utilized to distinguish the AP effect from the \DIFdelbegin \DIFdel{other effects, such as the redshift-space distortion (RSD).
}\DIFdelend \DIFaddbegin \DIFadd{effect of RSD.
}

\DIFaddend The radial distances of galaxies are inferred from \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{their }\DIFaddend measured redshifts, 
which are distorted \DIFdelbegin \DIFdel{by }\DIFdelend \DIFaddbegin \DIFadd{due to }\DIFaddend the peculiar motion of galaxies\DIFdelbegin \DIFdel{, 
leading to }\DIFdelend \DIFaddbegin \DIFadd{. 
This leads to an }\DIFaddend apparent distortion in the \DIFdelbegin \DIFdel{redshift space }\DIFdelend \DIFaddbegin \DIFadd{redshift-space }\DIFaddend galaxy distribution \citep{FOG,Kaiser1987,Ballinger1996}. 
\DIFdelbegin \DIFdel{It is the major systematic effects }\DIFdelend \DIFaddbegin \DIFadd{This RSD effect is the main systematic }\DIFaddend limiting our ability to probe the geometry of LSS from the galaxy distribution. 
However, we found that the influence of RSD is significantly reduced when \DIFdelbegin \DIFdel{we consider only }\DIFdelend \DIFaddbegin \DIFadd{focusing on }\DIFaddend the redshift dependence of \DIFdelbegin \DIFdel{the distortion.
}\DIFdelend \DIFaddbegin \DIFadd{galaxy clustering. 
}\DIFaddend 

\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Li2014} }%DIFAUXCMD
attempted to use the }%DIFDELCMD < {\it %%%
\DIFdel{galaxy density gradient field}%DIFDELCMD < } 
%DIFDELCMD < %%%
\DIFdel{to characterize the redshift evolution of the large scale galaxy distribution. 
We tested the idea on the Horizon Run 3 (HR3) N-body simulations \mbox{%DIFAUXCMD
\citep{horizonrun}}%DIFAUXCMD
,
...
The same topic was revisited in \mbox{%DIFAUXCMD
\cite{Li2015}}%DIFAUXCMD
, using a slightly modified version of the usual anisotropic galaxy }\DIFdelend \DIFaddbegin \DIFadd{In \mbox{%DIFAUXCMD
\cite{Li2016}}%DIFAUXCMD
, we analyzed the }\DIFaddend two-point correlation function (2pCF) \DIFdelbegin \DIFdel{, $\xi(\mu)$, 
...
The method was applied to BOSS DR12 galaxies \mbox{%DIFAUXCMD
\citep{Li2016} }%DIFAUXCMD
to probe }\DIFdelend \DIFaddbegin \DIFadd{of galaxies measured in the Baryon Oscillation Spectroscopic Survey (BOSS). 
We showed that }\DIFaddend the redshift dependence of the AP effect \DIFdelbegin \DIFdel{, 
and we arrive at }\DIFdelend \DIFaddbegin \DIFadd{leads to }\DIFaddend tight cosmological constraints \DIFdelbegin \DIFdel{competitive with the mainstream probes of SNIa, BAO, and CMB}\DIFdelend \DIFaddbegin \DIFadd{that are competitive with mainstream cosmological probes such as type Ia supernovae (SNIa), baryon acoustic oscillations (BAO), and cosmic microwave background (CMB)}\DIFaddend .


In this paper we continue our exploration of \DIFdelbegin \DIFdel{this strategy }\DIFdelend \DIFaddbegin \DIFadd{redshift invariant galaxy clustering 
}\DIFaddend and present a new method \DIFaddbegin \DIFadd{for constraining cosmology }\DIFaddend using the redshift evolution of 
the \DIFdelbegin \DIFdel{shape of the galaxy }\DIFdelend \DIFaddbegin \DIFadd{galaxy two-point correlation function (}\DIFaddend 2pCF\DIFaddbegin \DIFadd{) }\DIFaddend across the line-of-sight\DIFdelbegin \DIFdel{as a function of radial scale}\DIFdelend , $\xi(r_{\bot})$\DIFdelbegin \DIFdel{,
to constrain cosmology. 
A wrongly adopted }\DIFdelend \DIFaddbegin \DIFadd{. 
An incorrect choice of }\DIFaddend cosmology results in {\it redshift-dependent \DIFdelbegin \DIFdel{mis-scaling}\DIFdelend \DIFaddbegin \DIFadd{scaling}\DIFaddend } of the galaxy distribution\DIFdelbegin \DIFdel{,
which leads to redshift-evolution of }\DIFdelend \DIFaddbegin \DIFadd{. 
This, in turn, results in a significant redshift evolution in }\DIFaddend the shape of $\xi(r_{\bot})$.
\DIFdelbegin \DIFdel{The signal is sensitive to the cosmological parameters but relatively insensitive to the other effects }\DIFdelend \DIFaddbegin \DIFadd{Of course the correlation function has other contributions factors that may induce redshift evolution, 
}\DIFaddend such as the gravitational growth of structure, \DIFdelbegin \DIFdel{the }\DIFdelend galaxy bias, and \DIFdelbegin \DIFdel{the RSD effect. 
We apply our method to HR4 mock catalogues, using hundreds of millions of }\DIFdelend \DIFaddbegin \DIFadd{redshift space distortions. 
However we find that these other contributions do not produce large redshift evolution in the shape of $\xi(r_{\bot})$. 
We test this methodology using a sample of 1.75 billion mock }\DIFaddend galaxies at redshifts \DIFdelbegin \DIFdel{$0\leq z\leq2$
and develop the statistical procedure to constrain cosmological parameters}\DIFdelend \DIFaddbegin \DIFadd{0, 0.5, 1, 1.5, 2, drawn from the Horizon Run (HR) 4 N-body simulation}\DIFaddend .


%DIF < In this paper we continue our series of work and probe to use the angular correlation function as a statistical tool to probing the volume effect in the angular direction.
%DIF < The method of redshift dependent AP effect has been shown great power to constrain cosmological parameters \cite{Li2014,Li2015,Li2015}.
%DIF < We hope this method also.
%DIF < The two methods probe different information encoded in the 2pCF and can be combined to fully probe the geometrical effect in LSS.
%DIF < Or one can also use the redshift evolution of 2D 2pCF xi(s,mu) to have all information included.
%DIF < We developed a method measuring the redshift of the shape of galaxy angular correlation function, $\hat \xi_{r_\perp}$, to constrain cosmology.
%DIF < A wrongly adopted cosmology result in redshift-dependent mis-scaling in the constructed galaxy distribution,
%DIF < which shifts apparent scale of the clustering patterns;
%DIF < this manifests itself as a redshift-dependent rescaling of the measured angular correlation function.
%DIF < We find this signal is sensitive to the adopted cosmological parameters and relatively insensitive to the gravitational growth of structure,
%DIF < the galaxy bias, and the RSD effect.
%DIF < We test our method on the HR4 mock galaxies.
%DIF < The method can be applied to current and future LSS surveys,
%DIF < especially, the photometric surveys such as DES and LSST which probe a large amount of galaxies and the large redshift error will not quite affect the angular 2pCF measurement.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The outline of this paper proceeds as follows. 
...
In \S 3 we describe the N-body mock galaxies used \DIFdelbegin \DIFdel{to test our method}\DIFdelend \DIFaddbegin \DIFadd{in the analysis}\DIFaddend .
The methodology is presented in \S \DIFdelbegin \DIFdel{4.
}\DIFdelend \DIFaddbegin \DIFadd{4 and \S 5.
}\DIFaddend We conclude in \S \DIFdelbegin \DIFdel{5.
...
\begin{figure*}
   ...
   \DIFdelbeginFL \DIFdelFL{A description }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{An illustrative example }\DIFaddendFL of how incorrectly assumed cosmologies can distort \DIFdelbeginFL \DIFdelFL{the interpretation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a distribution }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{observation}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{points}\DIFaddendFL ,
   ...
   Their true positions and shapes are plotted \DIFdelbeginFL \DIFdelFL{in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{as }\DIFaddendFL blue dashed lines.
   When the observer adopts incorrect cosmologies when computing distances from redshifts and \DIFdelbeginFL \DIFdelFL{infering }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{inferring }\DIFaddendFL the positions and shapes of the squares,
...
In this section we briefly \DIFdelbegin \DIFdel{introduce }\DIFdelend \DIFaddbegin \DIFadd{discuss }\DIFaddend the scaling effect \DIFdelbegin \DIFdel{caused by incorrectly assumed }\DIFdelend \DIFaddbegin \DIFadd{in the galaxy clustering statistics caused by assuming incorrect }\DIFaddend cosmological parameters. A more detailed description \DIFdelbegin \DIFdel{has been provided }\DIFdelend \DIFaddbegin \DIFadd{can be found }\DIFaddend in \cite{Li2014,Li2015,Li2016}.
...
Cosmological models (i,iii) result in compressed volume elements (in both \DIFaddbegin \DIFadd{the }\DIFaddend angular and LOS directions),
...
In \DIFdelbegin \DIFdel{sum}\DIFdelend \DIFaddbegin \DIFadd{summary}\DIFaddend , as a consequence of \DIFdelbegin \DIFdel{incorrect }\DIFdelend \DIFaddbegin \DIFadd{incorrectly assumed }\DIFaddend cosmologies 
the size of objects is mis-estimated and the magnitude of mis-estimation depends on the redshift.
We \DIFaddbegin \DIFadd{thus }\DIFaddend use the galaxy 2pCF across the line-of-sight to probe the mis-estimation of angular size, ${\Delta r_{\perp, \rm wrong}}/{\Delta r_{\perp, \rm true}}$.


\section{The Simulation Data}\label{sec:data}

We test the method using the mock \DIFdelbegin \DIFdel{galaxy samples }\DIFdelend \DIFaddbegin \DIFadd{galaxies }\DIFaddend produced by the Horizon Run 4  (HR4) N-body simulation \citep{hr4,hong2016}.

HR4 was made \DIFdelbegin \DIFdel{within a cube of volume }\DIFdelend \DIFaddbegin \DIFadd{in a }\DIFaddend $({3.15}\ h^{-1}\rm Gpc)^3$ \DIFaddbegin \DIFadd{cube }\DIFaddend using  $6300^3$ particles with mass $m_p\simeq 9 \times 10^9 \hMsun$.
...
The resulting mock galaxies were found to \DIFaddbegin \DIFadd{well }\DIFaddend reproduce the 2pCF of the \DIFdelbegin \DIFdel{SDSS DR7 }\DIFdelend volume-limited galaxy sample \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{zehavi2011} }%DIFAUXCMD
very well}\DIFdelend \DIFaddbegin \DIFadd{from Sloan Digital Sky Survey Data Release 7 (SDSS DR7) \mbox{%DIFAUXCMD
\citep{zehavi2011}}%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbegin \DIFdel{We take the snapshot data of the }\DIFdelend \DIFaddbegin \DIFadd{In this analysis, we use five snapshots of }\DIFaddend mock galaxies at $z=0,0.5,1,1.5,2$.
...
%$1.46\times 10^{-2}$, $1.30\times 10^{-2}$, $1.13\times 10^{-2}$, $0.98\times 10^{-2}$ and $0.73\times 10^{-2}\ (h^{-1} \rm Mpc)^{-3}$,
respectively.
\DIFdelbegin \DIFdel{Applying a uniform mass cut, at higher redshiftwe obtained a }\DIFdelend \DIFaddbegin \DIFadd{At higher redshift, we have }\DIFaddend smaller number of galaxies\DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{, with a }\DIFaddend {\it larger bias}.
\DIFaddbegin \DIFadd{The total number of galaxies is 1.75 billion.
}\DIFaddend 


As an illustration, Figure \ref{fig_scatter} displays \DIFdelbegin \DIFdel{the 2D distribution of }\DIFdelend a subsample of \DIFdelbegin \DIFdel{mock galaxies in five redshifts,
with }\DIFdelend \DIFaddbegin \DIFadd{galaxies at the five redshifts.
Their }\DIFaddend $x$, $y$ coordinates \DIFdelbegin \DIFdel{computed using }\DIFdelend \DIFaddbegin \DIFadd{are computed using two sets of cosmological parameters,
}\DIFaddend the ``correct'' \DIFdelbegin \DIFdel{cosmological parameters }\DIFdelend \DIFaddbegin \DIFadd{cosmology }\DIFaddend $\Omega_m=0.26,\ w=-1$ (left panels)\DIFaddbegin \DIFadd{,
}\DIFaddend and a set of wrong cosmological parameters $\Omega_m=0.05,\ w=-1.5$ (right panels), respectively.

When the correct cosmology is adopted\DIFaddbegin \DIFadd{, }\DIFaddend the cosmic scale is correctly estimated,
\DIFaddbegin \DIFadd{and }\DIFaddend the main factor \DIFdelbegin \DIFdel{leads to }\DIFdelend \DIFaddbegin \DIFadd{for the }\DIFaddend redshift evolution of galaxy distribution is the gravitational growth of structure.
With decreasing redshift, \DIFdelbegin \DIFdel{we clearly see }\DIFdelend the clusters and filaments form and grow.
On the other hand, \DIFdelbegin \DIFdel{when }\DIFdelend the wrong cosmology \DIFdelbegin \DIFdel{is adopted, there is }\DIFdelend \DIFaddbegin \DIFadd{leads to }\DIFaddend an artificial scaling of \DIFdelbegin \DIFdel{distances in the constructed map}\DIFdelend \DIFaddbegin \DIFadd{distance}\DIFaddend .
The separations among galaxies are over-estimated by 
$25.6\%,47.3\%,62.2\%,71.7\%$ at redshifts of $0.5,1.0,1.5,2.5$,
\DIFdelbegin \DIFdel{leading to }\DIFdelend \DIFaddbegin \DIFadd{causing }\DIFaddend a clear redshift evolution of \DIFaddbegin \DIFadd{the }\DIFaddend sizes of structures.
...
\DIFdelbegin \DIFdel{The imprint of each of these effects on the large scale structure is differentand should be distinguishable}\DIFdelend \DIFaddbegin \DIFadd{Their imprints are different}\DIFaddend .
In the \DIFdelbegin \DIFdel{following section}\DIFdelend \DIFaddbegin \DIFadd{next section, }\DIFaddend we show that they affect $\xi(r_{\bot})$ measurements in different \DIFdelbegin \DIFdel{ways}\DIFdelend \DIFaddbegin \DIFadd{manners}\DIFaddend , 
...
\begin{figure*}
  ...
  \DIFdelbeginFL \DIFdelFL{The 2D distribution }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$x$, $y$ coordinates }\DIFaddendFL of a $260\times130\times105 (h^{-1}{\rm Mpc})^3$ subsample of HR4 \DIFdelbeginFL \DIFdelFL{mock }\DIFdelendFL galaxies, 
  at redshifts 0.5, 1, 1.5 and 2.
  Left and right panels show the \DIFdelbeginFL \DIFdelFL{$x$, $y$ }\DIFdelendFL coordinates computed using the ``correct'' parameters $\Omega_m=0.26,\ w=-1$ 
  and a \DIFdelbeginFL \DIFdelFL{set of }\DIFdelendFL wrong \DIFdelbeginFL \DIFdelFL{cosmological parameters }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cosmology }\DIFaddendFL $\Omega_m=0.05,\ w=-1.5$, respectively.
...
\DIFdelbegin \DIFdel{The mis-scaling }\DIFdelend \DIFaddbegin \DIFadd{This scaling }\DIFaddend not only changes the shape of the 2pCF but also changes the value of $ \xi_{r_\perp}$. %The amplitude of $r_\perp\xi$ is amplified due to the stretch of scale.
...
It \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend the major systematic in our previous works \citep{Li2014,Li2015,Li2016}
...
the only effect of RSD enter in the \DIFdelbegin \DIFdel{procedure of splitting galaxies into redshift shells}\DIFdelend \DIFaddbegin \DIFadd{redshift distribution of the galaxies in our predefined redshift shell}\DIFaddend .
...
%DIF < estimated from the 72 sets of PSB mock galaxies identified from HR3 N-body simulation.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Figure \ref{fig_cosmo} displays the 2pCF and the likelihood values when adopting 
...
The two parameters \DIFdelbegin \DIFdel{are very strongly degenerated }\DIFdelend \DIFaddbegin \DIFadd{display a strongly }\DIFaddend with each other,
and the area of \DIFaddbegin \DIFadd{the }\DIFaddend constrained region is quite small.
The thin shape of the contour implies that, 
when combining with other observational data with a different \DIFdelbegin \DIFdel{direction of degeneracy }\DIFdelend \DIFaddbegin \DIFadd{degeneracy directions }\DIFaddend (e.g. CMB, BAO),
a very tight combined constraint \DIFdelbegin \DIFdel{can }\DIFdelend \DIFaddbegin \DIFadd{could }\DIFaddend be obtained.
...
Finally, to \DIFdelbegin \DIFdel{be connected with the real observations}\DIFdelend \DIFaddbegin \DIFadd{put the constraints of our new methodology in perspective}\DIFaddend , 
...
The predicted constraints from this method clearly \DIFdelbegin \DIFdel{wins }\DIFdelend \DIFaddbegin \DIFadd{has an advantage }\DIFaddend due to the large sample size.
The total number of galaxies used in this work is \DIFdelbegin \DIFdel{1,749 million}\DIFdelend \DIFaddbegin \DIFadd{1.75 billion}\DIFaddend ,
which is 1,000 times of the BOSS DR12 sample size.
So the constrained area is also $\sim 30$ times smaller.
Such a large sample size will be \DIFdelbegin \DIFdel{enabled by }\DIFdelend \DIFaddbegin \DIFadd{possible with }\DIFaddend LSST (the Large Synoptic Survey Telescope) \footnote{https://www.lsst.org/}, 
...
We have developed a method for constraining certain cosmological parameters by measuring the redshift evolution of the shape of the galaxy 2pCF across the \DIFdelbegin \DIFdel{line-of-sight}\DIFdelend \DIFaddbegin \DIFadd{LOS}\DIFaddend , $\hat \xi_{r_\perp}$.
We found that a wrongly adopted cosmology results in a redshift-dependent \DIFdelbegin \DIFdel{mis-scaling }\DIFdelend \DIFaddbegin \DIFadd{scaling }\DIFaddend in the constructed galaxy distribution,
...
We tested our method on the HR4 mock galaxies having 457, 406, 352, 306 and 228 million galaxies at redshifts 0, 0.5, 1, 1.5, \DIFaddbegin \DIFadd{and }\DIFaddend 2.
...
In this analysis we restrict our focus to the rescaling in the direction perpendicular to \DIFdelbegin \DIFdel{line-of-sight }\DIFdelend \DIFaddbegin \DIFadd{LOS}\DIFaddend .
...
as a function of both angular and radial \DIFdelbegin \DIFdel{scale }\DIFdelend \DIFaddbegin \DIFadd{scales }\DIFaddend to fully probe the scaling effect in the 3D galaxy distribution.

In our early work of \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Li2015,Li2016}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Li2015}}%DIFAUXCMD
}\DIFaddend , 
we proposed to use the \DIFdelbegin \DIFdel{angular-dependence }\DIFdelend \DIFaddbegin \DIFadd{$\mu$-dependence }\DIFaddend of the galaxy clustering to apply an AP test\DIFdelbegin \DIFdel{and derive cosmological constraints }\DIFdelend \DIFaddbegin \DIFadd{. 
Tight cosmological constraints are derived from the BOSS galaxies \mbox{%DIFAUXCMD
\citep{Li2016}}%DIFAUXCMD
}\DIFaddend . 
In contrast, in this work we focus on the \DIFdelbegin \DIFdel{scale-dependence of the }\DIFdelend \DIFaddbegin \DIFadd{$s$-dependence of galaxy }\DIFaddend clustering. 
...
However, it is advisable to have an estimation of the covariance between them in the case \DIFdelbegin \DIFdel{there }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend one is using both methods in combination
\DIFdelbegin \DIFdel{.
An alternative way is to investigate the redshift evolution of the correlation map $\xi(s,\mu)$ as a function of both scale and direction, which captures all the information}\DIFdelend \DIFaddbegin \footnote{\DIFadd{An alternative way is to investigate the redshift evolution of the correlation map $\xi(s,\mu)$ as a function of both scale and direction, which captures all the information.}}\DIFaddend .
...
Our method is simpler and just uses the fact that in the correct cosmology the shape of the 2pCF \DIFdelbegin \DIFdel{across the line-of-sight
}\DIFdelend does not exhibit significant redshift evolution.
...
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Li2015,Li2016} }%DIFAUXCMD
shows that using the redshift dependence of the AP effect, 
...
}\DIFdelend %DIF > \cite{Li2015,Li2016} shows that using the redshift dependence of the AP effect, 
%DIF > one can derive very tight cosmological constraints.
%DIF > The method presented in this paper studies the redshift evolution of the volume effect.
%We hope this method also.
\DIFdelbegin \DIFdel{These two methods probe different information encoded in the 2pCF 
and can be combined to give a complete geometric probe of the LSS galaxy distribution.
}\DIFdelend %DIF > These two methods probe different information encoded in the 2pCF 
%DIF > and can be combined to give a complete geometric probe of the LSS galaxy distribution.
...
The galaxy 2pCF across the \DIFdelbegin \DIFdel{line-of-sight in redshift shells is much }\DIFdelend \DIFaddbegin \DIFadd{LOS could be }\DIFaddend less affected by the large redshift errors\DIFdelbegin \DIFdel{compared wit the 3D 2pCF}\DIFdelend .
...
are introducing a new strategy \DIFdelbegin \DIFdel{to constrain cosmology from galaxy clustering }\DIFdelend \DIFaddbegin \DIFadd{of galaxy clustering statistics}\DIFaddend .
We utilize a statistical pattern which remains constant at all redshifts\DIFdelbegin \DIFdel{;
this invariance is broken when }\DIFdelend \DIFaddbegin \DIFadd{.
When }\DIFaddend incorrect cosmologies are adopted to construct the galaxy distribution\DIFdelbegin \DIFdel{and introduces redshift evolution of geometric distortion}\DIFdelend \DIFaddbegin \DIFadd{, it introduces redshift-dependent geometric distortion,
and the invariance is broken}\DIFaddend .
...
and enables reliable cosmological \DIFdelbegin \DIFdel{constraint }\DIFdelend \DIFaddbegin \DIFadd{constraints }\DIFaddend on relatively small clustering scales.
