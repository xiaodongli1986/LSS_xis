...
We found fact that the AP or volume effect in general introduces a geometric distortion that evolves significantly with redshift;
this phenomenon can be utilized to distinguish the \scr{AP effect(?)} the from the other effects such as the redshift space distortion (RSD).
...
can be used to infer unbiased constraints on certain cosmological parameters and that it is, largely, insensitive to RSD.
...
The RSD effect, significantly distorts $\xi(\mu)$, however it
exhibits much less redshift evolution.
...
The growth of structure enhances the clustering and results in more compact structures,
while the volume effect maintains the clustering pattern and uniformly re-scales the structures on all scales.
...
while the covariances of them are taken to be the covariance matrix (after multiplying a factor of $1/\sqrt{119}$).
...
Apart from the large variation in amplitude between the different redshifts bins, 
%The shape of $r_\perp\omega$ also evolves with redshift.
the shape of $r_\perp\omega$  is similar at all redshifts;
...
In the upper-middle panel, the overlapping $\hat\omega$'s
...
one would significantly underestimate the statistical uncertainty of $\delta \hat{\omega}_{r_\perp}$ by ignoring the cosmic variance.
...
constant mass cut samples at different redshifts are relatively close to each other.
...
such as mass, morphology, color, concentration, and so on. 
...
and the slop of $\hat\omega_{r_\perp}$ is slightly suppressed.
